...
- `/news-workflow [브랜치명]` — 새 기능 개발 시작부터 커밋, main 병합까지 안내하는 워크플로우 명령어 (`.claude/commands/news-workflow.md`). main 브랜치 상태 확인 → 새 브랜치 생성 → 기술 스택/컨벤션 안내 → 작업 완료("완료"/"done") 시 커밋 메시지 제안 및 main 병합 → 결과 요약 순서로 진행됩니다.
